...
router.get("/", async (request, response) => {
    const collection = await invoice_1.default.find({});
...
router.get("/:id", async (request, response) => {
    ...
    const result = await invoice_1.default.find({ _id: id });
    if (result) {
...
router.post("/", async (request, response) => {
    ...
    const appt = await appointment_1.default.findById(body.appointment);
    if (!appt) {
        return response.status(400).json({ error: "apptId missing or not valid" });
    }
    const patient = await patient_1.default.findById(appt.patient);
    if (!patient) {
        return response
            .status(400)
    ...
    const item = new invoice_1.default(body);
...
router.put("/:id", async (request, response) => {
    ...
    const body = request.body;
    const result = await invoice_1.default.findOneAndUpdate({ _id: { $eq: id } }, body, {
        new: true,
    });
...
router.delete("/:id", async (request, response) => {
    ...
    const id = request.params.id;
    const result = await invoice_1.default.findOneAndDelete({ _id: { $eq: id } });
