...
router.get("/:id", async (request, response) => {
    ...
    const result = await position_1.default.find({ _id: id });
    if (result) {
...
router.post("/", async (request, response) => {
    ...
    const item = new position_1.default(body);
...
router.put("/:id", async (request, response) => {
    ...
    const body = request.body;
    const result = await position_1.default.findOneAndUpdate({ _id: { $eq: id } }, body, {
        new: true,
    });
...
router.delete("/:id", async (request, response) => {
    ...
    const id = request.params.id;
    const result = await position_1.default.findOneAndDelete({ _id: { $eq: id } });
