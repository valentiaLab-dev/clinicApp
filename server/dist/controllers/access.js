...
router.get("/:id", async (request, response) => {
    ...
    const result = await access_1.default.find({ _id: id });
    if (result) {
...
router.put("/:id", async (request, response) => {
    ...
    const body = request.body;
    const result = await access_1.default.findOneAndUpdate({ _id: { $eq: id } }, body, {
        new: true,
    });
...
router.delete("/:id", async (request, response) => {
    ...
    const id = request.params.id;
    const result = await access_1.default.findOneAndDelete({ _id: { $eq: id } });
